...
- **Шаблон**: особая «поездка» с `kind:'template'` и фиксированным `id:'__tpl__'`
  (всегда одна, см. `ensureTemplate()`). Без дат, не удаляется, показывается отдельной
  карточкой сверху на экране поездок. При первом создании автозаполняется из текущего/
  первого списка. Доступен как источник «Шаблон» при создании поездки. Хелперы:
  `getTemplate()`, `realTrips()`, `isTemplate()`.
